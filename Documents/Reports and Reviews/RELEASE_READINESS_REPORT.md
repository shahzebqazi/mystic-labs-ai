...
**Last updated:** Aligned with current repository layout (Project/Orchestration/Tasks/...).
...
|-----------|------|--------|-------|
| **Infrastructure** | docker-compose (LB, Web Tier, DB, Redis, production-logs, Prometheus, Grafana) | Scaffolded | Paths under `Project/Orchestration/Tasks/INFRA/`; compose validated when Docker available |
| | Terraform | Stub | `Project/Orchestration/Tasks/INFRA/terraform/main.tf`; modules not wired |
| | Nginx LB config | Exists | `Project/Orchestration/Tasks/INFRA/nginx/nginx.conf` |
| | Prometheus scrape | Exists | `Project/Orchestration/Tasks/INFRA/prometheus/prometheus.yml` |
| **CI/CD** | GitHub Actions: lint, unit test | Wired | Paths point at `Project/Orchestration/Tasks/SWE/api`; run on push/PR |
| | Blue-Green deployment workflow | Placeholder | Build context and paths updated; deploy step is placeholder |
| | docker-compose config validation | Wired | Uses `Project/Orchestration/Tasks/INFRA/docker-compose.yml` |
...
| **Compliance** | DB schema for encryption at rest | Exists | `Project/Orchestration/Tasks/Compliance/init-db-encryption.sql` |
...
| **Compliance (SOC2/GDPR)** | `Project/Orchestration/Tasks/Compliance/init-db-encryption.sql`, encrypt-at-rest doc, encrypt helper for PII |
...
<repo root>/
├── .github/workflows/
│   ├── ci.yml                    # Lint + test (Project/Orchestration/Tasks/SWE/api)
...
- **API:** `npm run lint` and `npm test` in `Project/Orchestration/Tasks/SWE/api` pass (NODE_ENV=test for tests).
- **Automation:** CI workflow paths reference the above layout; compose file paths are relative to `Project/Orchestration/Tasks/INFRA/`.
