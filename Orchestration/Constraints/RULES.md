...
5. **Feature branch per task** — AI agents must create a **feature branch for each task** they work on. All work for that task must happen on that branch. See [JJ.md](Orchestration/Tasks/VCS/JJ.md) (§ Feature branch per task). Applies to every agent (orchestrator-spawned, chatbot, Cursor, Codex, etc.).
